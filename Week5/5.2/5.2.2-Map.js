...
let newData = data.map(el => el.address.city);
console.log(newData);

// create an array of latitude and longitude coordinate pairs
let latLong = data.map(el => {
  return [el.address.geo.lat,el.address.geo.lng];
});
console.log(latLong);

// create an array of strings that combine the company's catchPhrase and bs
let stringArr = data.map(el => `${el.company.catchPhrase} - ${el.company.bs}`);
