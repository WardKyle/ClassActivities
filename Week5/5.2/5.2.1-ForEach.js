...
let newArr = [];
data.forEach(el => {
  let lat = el.address.geo.lat;
  let lng = el.address.geo.lng;
  newArr.push([lat,lng]);
});
console.log(newArr);
...
let arr2 = [];
data.forEach(el => {
  arr2.push(`${el.company.catchPhrase} - ${el.company.bs}`);
});
console.log(arr2);
...
classWeek.forEach((el,i) => {
  el.innerHTML = `Updated el is at index: ${i}`;
});
